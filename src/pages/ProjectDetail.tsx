...
// Function to download case study as PDF
const downloadCaseStudy = (project: ProjectDetail) => {
  const doc = new jsPDF();
  ...
  const titleLines = doc.splitTextToSize(project.title, pageWidth - 40);
  ...
  doc.text(`Year: ${project.year}`, 20, yPos);
  yPos += 6;
  if (project.location) {
    doc.text(`Location: ${project.location}`, 20, yPos);
    yPos += 6;
  }
  doc.text(`Categories: ${project.categories.join(', ')}`, 20, yPos);
  yPos += 12;

  // Content
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  const contentLines = doc.splitTextToSize(textContent, pageWidth - 40);
  ...
  doc.setFontSize(11);
  doc.setFont('helvetica', 'normal');
  Object.entries(project.key_metrics).forEach(([key, value]) => {
    if (yPos > 270) {
      doc.addPage();
      yPos = 20;
    }
    doc.text(`• ${key.replace(/_/g, ' ')}: ${value}`, 25, yPos);
    yPos += 6;
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main>
        ...
        <section ref={heroRef} className="relative min-h-screen flex items-center overflow-hidden py-24">
            ...
            <div className="max-w-6xl mx-auto">
              ...
              <div className={`flex flex-wrap gap-3 mb-8 transition-all duration-700 delay-100 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-1 translate-y-4'}`}>
                {project.categories.map((category, index) => (
                  <span
                    key={category}
                    className="px-5 py-2.5 rounded-full bg-gradient-to-r from-secondary/20 to-accent/20 backdrop-blur-md text-secondary text-sm font-bold border border-secondary/30"
              ...
              <div className={`transition-all duration-700 delay-400 ${heroVisible ? 'opacity-100 translate-y-0' : 'opacity-1 translate-y-4'}`}>
                <div className="text-center mb-8">
                  <div className="inline-flex items-center gap-2 px-5 py-2.5 mb-4 text-sm font-bold text-secondary bg-secondary/20 rounded-full border border-secondary/30 backdrop-blur-md">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  {Object.entries(project.key_metrics).map(([key, value], index) => (
                    <div
                      key={key}
                      className="group relative p-6 rounded-2xl bg-white/10 backdrop-blur-xl border border-white/20 hover:border-secondary/50 transition-all duration-500 hover:-translate-y-1 text-center"
          ...
          {/* Decorative Elements */}
          <div className="absolute top-1/4 left-1/4 w-96 h-96 bg-secondary/10 rounded-full blur-3xl z-10" />
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
            <div className="max-w-5xl mx-auto">
              ...
              <div className="text-center mb-16">
                <div className="inline-flex items-center gap-2 px-5 py-2.5 mb-6 text-sm font-bold text-secondary bg-secondary/10 rounded-full border border-secondary/20">
              ...
              <div className="p-8 md:p-12 lg:p-16 rounded-3xl bg-card/40 backdrop-blur-xl border border-border/50 shadow-2xl">
                ...
                <article className="prose prose-xl max-w-none text-center
                  ...
                  prose-h3:before:bg-gradient-to-b prose-h3:before:from-secondary prose-h3:before:to-accent 
                  ...
                  prose-li:pl-3 prose-li:marker:text-secondary prose-li:marker:text-2xl
                  prose-strong:text-foreground prose-strong:font-bold
                  prose-strong:bg-gradient-to-r prose-strong:from-secondary/10 prose-strong:to-transparent
                ...
                <div className="mt-20 pt-8 border-t border-border/50">
                  ...
                  <div className="flex flex-wrap gap-3 justify-center">
                    {project.tech_stack.map((tech, index) => (
                      <span
                        key={tech}
                        className="px-5 py-3 rounded-xl bg-gradient-to-br from-secondary/10 to-accent/10 
                          border border-secondary/20 text-foreground font-semibold text-sm
        ...
        {/* Download CTA */}
        <section className="py-20 bg-gradient-to-br from-secondary/10 via-accent/10 to-secondary/10">
            ...
            <div className="max-w-4xl mx-auto">
              <div className="p-12 rounded-3xl bg-gradient-to-br from-card via-card/95 to-card/90 backdrop-blur-xl border border-border/50 shadow-2xl text-center">
                ...
                <Button
                  ...
                  className="bg-gradient-to-r from-secondary to-accent hover:opacity-90 text-white font-bold text-lg px-8 shadow-lg shadow-secondary/30"
                >
          ...
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="max-w-4xl mx-auto text-center">
              ...
              <div className="flex justify-center">
                <Button
                  size="lg"
                  className="bg-gradient-to-r from-secondary to-accent hover:opacity-90 text-white font-bold text-lg px-10 py-6 shadow-xl shadow-secondary/30"
                  asChild
                >
                  <a href="/projects">
                    View More Projects
